...
func New() *router.Root {
	db := database.New()
	iGameRepository := repository.NewGameRepository(db)
	iGameService := service.NewGameService(iGameRepository)
	iGameHandler := handler.NewGameHandler(iGameService)
	root := router.New(iGameHandler)
	return root
}
